...
@section('content')
    <div class="p-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">Edit Promotion</h1>
        ...
        <form action="{{ route('promotions.update', $promotion) }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="products[]" id="products" class="w-full" multiple>
                    @foreach ($products as $product)
                        <option value="{{ $product->id }}" {{ $promotion->products->contains($product->id) ? 'selected' : '' }}>{{ $product->name }}</option>
                    @endforeach
                </select>
                @error('products')
            ...
            <div class="mb-6">
                ...
                <select name="categories[]" id="categories" class="w-full" multiple>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ $promotion->categories->contains($category->id) ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
                @error('categories')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            ...
            <div class="mb-6">
                ...
                <select name="type" id="type" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    <option value="buy_get_free" {{ $promotion->type == 'buy_get_free' ? 'selected' : '' }}>Buy and Get Free</option>
                </select>
                @error('type')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                @enderror
            ...
            <!-- Discount Fields -->
            <div class="discount-fields fade-transition mb-6 {{ $promotion->type == 'discount' ? 'show' : '' }}">
            ...
            <div class="discount-fields fade-transition mb-6 {{ $promotion->type == 'discount' ? 'show' : '' }}">
                ...
                <input type="number" name="discount_value" id="discount_value" value="{{ $promotion->discount_value }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" step="0.01" min="0">
            ...
            <!-- Free Item -->
            <div class="free-item-fields fade-transition mb-6 {{ $promotion->type == 'buy_get_free' ? 'show' : '' }}">
                ...
                <select name="free_item_id" id="free_item_id" class="w-full">
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->id }}" {{ $promotion->free_item_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                    @endforeach
                </select>
                @error('free_item_id')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="start_date" id="start_date" value="{{ $promotion->start_date->format('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                @error('start_date')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="end_date" id="end_date" value="{{ $promotion->end_date->format('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                @error('end_date')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="is_active" class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $promotion->is_active ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
