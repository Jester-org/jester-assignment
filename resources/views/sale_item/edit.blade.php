...
@section('content')
    ...
    <form action="{{ route('sale-items.update', $saleItem) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="number" name="quantity" id="quantity" class="form-control" min="1" value="{{ old('quantity', $saleItem->quantity) }}" required>
            @error('quantity')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        <div class="form-group">
            ...
            <input type="number" name="unit_price" id="unit_price" class="form-control" step="0.01" value="{{ old('unit_price', $saleItem->unit_price) }}" required>
            @error('unit_price')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        <div class="form-group">
            ...
            <input type="number" name="subtotal" id="subtotal" class="form-control" step="0.01" value="{{ old('subtotal', $saleItem->subtotal) }}" readonly>
            @error('subtotal')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
